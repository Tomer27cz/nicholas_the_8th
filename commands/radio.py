...
async def radio_cz_def(ctx, glob: GlobalVars, query: str, mute_response: bool=False) -> ReturnData:
    ...
    if url_type not in ['RadioCz', 'String']:
        message = txt(guild_id, glob, "Wrong command! Try ") + f"`/{command_for_type(url_type)}` {glob.notif}"
    ...
    if not radios:
        message = txt(guild_id, glob, "Radio **not found**") + f' {glob.notif}'
        if not mute_response:
            await ctx.reply(message)
        return ReturnData(False, message)
    ...
    message = txt(guild_id, glob, "Choose radio") + f' {glob.notif}'
